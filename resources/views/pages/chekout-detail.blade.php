                ...
                <div class="card shadow-sm mb-5">
                            ...
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    @foreach ($transaction->transactionItems as $item)
                                        <tr>
                                            ...
                                            <td>RP
                                                {{ number_format($item->price * $item->qty + $transaction->ongkir, 0, ',', '.') }}
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                                <tfoot>
                                    <tr>
                                        ...
                                        <td><strong>RP
                                                {{ number_format($transaction->transactionItems->sum(function ($item) {return $item->price * $item->qty;}),0,',','.') }}</strong>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td colspan="4" class="text-end"><strong>Ongkir:</strong></td>
                                        <td><strong>RP {{ number_format($transaction->ongkir, 0, ',', '.') }}</strong></td>
                                    </tr>
                                    <tr class="table">
                                        <td colspan="4" class="text-end"><strong>Total:</strong></td>
                                        <td><strong style="color: #B88E2F">RP
                                                {{ number_format($transaction->transactionItems->sum(function ($item) {return $item->price * $item->qty;}) + $transaction->ongkir,0,',','.') }}</strong>
                                        </td>
                                    </tr>
                                </tfoot>
                            </table>
                    ...
                    <div class="text-center">
                                ...
                                document.addEventListener('DOMContentLoaded', function() {
                                    ...
                                    function updateRemainingTime() {
                                        ...
                                        if (remainingTimeSeconds <= 0) {
                                            ...
                                            localStorage.removeItem('endTime_' + transactionId);
                                            // Mengatur ulang tombol dan waktu seperti awal
                                            document.getElementById('requestOngkirBtn').disabled = false;
                        ...
                        @else
                            ...
                            <a href="{{ route('checkout.download', $transaction->id) }}" class="btn btn-success">Download
                                Receipt</a>
                                ...
                                <tbody>
                                    @foreach ($ongoingTransactions as $trans)
                                        <tr>
                                            ...
                                            <td>
                                                <ul style="list-style: none;">
                                                    @foreach ($trans->transactionItems as $item)
                                                        @if ($item->product)
                                                            <li>{{ $item->product->name_product }} ({{ $item->qty }})
                                                            </li>
                                                        @else
                                            ...
                                            <td><a href="{{ route('checkout.detail', $trans->id) }}"
...
{{-- <script>
    document.addEventListener('DOMContentLoaded', function() {
        let timeLeftElement = document.getElepmentById('timeLeft');
        let btnTimeLeftElement = document.getElementById('btnTimeLeft');
        let timeLeft = parseInt(timeLeftElement.innerText) * 60;
        ...
        function updateCountdown() {
            if (timeLeft > 0) {
                ...
                btnTimeLeftElement.innerText = minutes;
            }
        }
